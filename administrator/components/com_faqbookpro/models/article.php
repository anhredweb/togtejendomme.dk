...
class FAQBookProModelArticle extends JModelAdmin
{
	...
	protected function canDelete($record)
	{
		if (!empty($record->id))
		{
			...
			$user = JFactory::getUser();
			return $user->authorise('core.delete', 'com_faqbookpro.article.' . (int) $record->id);
		}
	}
	...
	protected function canEditState($record)
	{
		...
		if (!empty($record->id))
		{
			return $user->authorise('core.edit.state', 'com_faqbookpro.article.' . (int) $record->id);
		}
		// New article, so check against the category.
		elseif (!empty($record->catid))
		{
			return $user->authorise('core.edit.state', 'com_faqbookpro.category.' . (int) $record->catid);
		}
