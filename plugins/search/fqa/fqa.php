...
require_once JPATH_SITE.'/components/com_faqbookpro/router.php';
 ...
 * @since		1.6
 */
class plgSearchFqa extends JPlugin
{
	/**
	...
	function onContentSearch($text, $phrase='', $ordering='', $areas=null)
	{
		...
		require_once JPATH_SITE . '/components/com_faqbookpro/helpers/route.php';
		...
		if ($sContent && $limit > 0)
		{
			...
			if (isset($list))
			{
				foreach($list as $key => $item)
				{
					...
					//$list[$key]->href = FaqBookProHelperRoute::getArticleRoute($item->slug, $item->catslug);

					$strUrl = FaqBookProHelperRoute::getArticleRoute($item->slug, $item->catslug);

					$list[$key]->href = str_replace('271', '432', $strUrl);

					// $list[$key]->href = $strUrl;

					//$list[$key]->href = JRoute::_('index.php?option=com_faqbookpro&view=item&id=' . $item->id . '&Itemid=271');
				}
			}
			$rows[] = $list;
		}
		...
		if ($sArchived && $limit > 0)
		{
			...
			$item	= $app->getMenu()->getItems('link', 'index.php?option=com_faqbookpro&view=archive', true);
			...
			if (isset($list3))
			{
				foreach($list3 as $key => $item)
				{
					...
					$list3[$key]->href	= JRoute::_('index.php?option=com_faqbookpro&view=archive&year='.$created_year.'&month='.$created_month.$itemid);
				}
			}
