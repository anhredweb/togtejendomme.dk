...
function FaqBookProBuildRoute(&$query)
{
	...
	if ($view == 'category' || $view == 'item' || $view == 'faqbook')
	{
		if (!$menuItemGiven)
		{
			$segments[] = $view;
		...
		// remove 1st level cat = remove last array element//////////////////////////////////////////////////
		unset($path[count($path)-1]); // Dimo na to 8umasai
		/////////////////////////////////////////////////////////////////////////////////////////////////////
...
function FaqBookProParseRoute($segments)
{
	...
	if ($count == 1)
	{
		...
		{
			$query = 'SELECT alias, catid FROM #__faqbookpro_content WHERE id = ' . (int) $id;
			$db->setQuery($query);
	...
	if (!$category)
	{
		JError::raiseError(404, JText::_('COM_FAQBOOKPRO_ERROR_PARENT_CATEGORY_NOT_FOUND'));
		return $vars;
	}
